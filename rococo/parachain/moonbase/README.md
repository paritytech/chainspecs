...
```shell
docker run --rm purestake/moonbeam:v0.21.0 build-spec --chain moonbase-dev > ./moonbase-dev-plain.json
# Fix network name
sed 's/"id": "moonbase_dev"/"id": "moonbase"/' -i ./moonbase-dev-plain.json
sed 's/"chainType": "Development"/"chainType": "Live"/' -i ./moonbase-dev-plain.json
sed 's/"relayChain": "dev-service"/"relayChain": "rococo"/' -i ./moonbase-dev-plain.json
# Update parachian id 
sed 's/"paraId": 0/"paraId": 2023/' -i ./moonbase-dev-plain.json
sed 's/"parachainId": 0/"parachainId": 2023/' -i ./moonbase-dev-plain.json
# Update bootnodes
sed 's,"/ip4/127.0.0.1/tcp/30333/p2p/.*","/ip4/34.91.156.128/tcp/30334/p2p/12D3KooWApeyhcqaAqiEZTPrAbTetUsoZWJNVBuDca2WfAX6cUUp",' -i ./moonbase-dev-plain.json
# Replace alice key with ROCOCO_VALIDATORS_ROOT_SEED (https://gitlab.parity.io/parity/infrastructure/parity-testnet-validator-manager/-/blob/e559036e81204c4c953bef0ee4edc7b12390c690/.gitlab-ci.yml#L72)
...
# replace bob key (eth_path = "/m/44'/60'/0'/0/1")
...
docker run --rm -v $(pwd)/moonbase-dev-plain.json:/moonbase-dev-plain.json purestake/moonbeam:v0.21.0 build-spec --chain /moonbase-dev-plain.json --raw > ./chainspec.json
rm ./moonbase-dev-plain.json
```
