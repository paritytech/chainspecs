...
PREFIX=rococo-coretime-collator-node-
...
for i in $(seq 0 $V_NUM); do
  ...
  BOOTNODES+="\"/dns/${PREFIX}${i}.polkadot.io/tcp/30333/p2p/${NODEID}\"${LAST}"
...
printf "$CHAINSPEC" | jq | sed 's/1e+18/1000000000000000000/' > ./coretime-override.json
printf "${BOOTNODES_KEY_LIST}" | sed 's/-/_/g' > ./coretime_nodekey.yml